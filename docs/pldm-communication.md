...
The `openpower-occ-control` application uses the Platform Level Data Model (PLDM)
protocol as the primary interface between the BMC and the host firmware (HBRT —
Hostboot Runtime) for two purposes:
...
4. Call `prepareSetEffecterReq()` to encode the `SetStateEffecterStates` message,
   setting only the target state field to the desired value and all others to
   `PLDM_NO_CHANGE`.
...
`(terminusID, sensorID, sensorOffset, eventState, previousEventState)`.
The `sensorOffset` is compared against the cached `OCCSensorOffset` and
...
- **Offset:** `OCCSensorOffset` — position of the running-status state set within
  the composite sensor PDR
...
`payload[0] == PLDM_SUCCESS`. A non-success response triggers an immediate retry.
